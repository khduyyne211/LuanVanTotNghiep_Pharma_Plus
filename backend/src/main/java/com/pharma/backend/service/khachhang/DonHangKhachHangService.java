...
import com.pharma.backend.dto.khachhang.donhang.ChiTietDonHangResponseDto;
import com.pharma.backend.dto.khachhang.donhang.DonHangDanhSachDto;
import com.pharma.backend.dto.khachhang.donhang.DonHangResponseDto;
import com.pharma.backend.dto.khachhang.donhang.SanPhamDonHangTomTatDto;
import com.pharma.backend.dto.khachhang.donhang.TaoDonHangRequestDto;
...
public class DonHangKhachHangService {
        ...
        public DonHangResponseDto taoDonHang(
                ...
                DuLieuTaoDonHang duLieu = chuanBiDuLieuTaoDonHang(
                                maKhachHang,
                                request);

                List<ChiTietGioHang> danhSachChiTietGioHang = duLieu.getDanhSachChiTietGioHang();

                /*
                 * Khách hàng không được tự tạo đơn có thuốc kê đơn.
                 * Kiểm tra lại tại bước tạo đơn để bảo vệ cả trường hợp
                 * dữ liệu giỏ hàng cũ hoặc dữ liệu đã tồn tại trước đó.
                 */
                kiemTraKhachHangKhongTuDatThuocKeDon(
                ...
                List<DonViSanPham> danhSachDonVi = layDanhSachDonViKhongTrung(
                                danhSachChiTietGioHang);
                ...
                BigDecimal tongTienHang = tinhTongTienHang(
                ...
                List<ChiTietDonHang> danhSachChiTietDaLuu = chiTietDonHangRepository.saveAll(
                                danhSachChiTietDonHang);
        ...
        public List<DonHangDanhSachDto> layDanhSachDonHang(
                ...
                List<DonHang> danhSachDonHang = donHangRepository
                ...
                Map<Long, List<ChiTietDonHang>> chiTietTheoMaDonHang = gomChiTietTheoMaDonHang(danhSachChiTiet);

                List<DonHangDanhSachDto> ketQua = new ArrayList<>();

                for (DonHang donHang : danhSachDonHang) {
                        List<ChiTietDonHang> chiTietCuaDon = chiTietTheoMaDonHang.getOrDefault(
        ...
        public void huyDonHang(
                ...
                boolean coTheHuyDonCod = donHang.getPhuongThucThanhToan() == PhuongThucThanhToan.COD
                                && donHang.getTrangThaiDonHang() == TrangThaiDonHang.CHO_XU_LY
                                && donHang.getTrangThaiThanhToan() == TrangThaiThanhToan.CHUA_THANH_TOAN;

                boolean coTheHuyDonZaloPay = donHang.getPhuongThucThanhToan() == PhuongThucThanhToan.ZALOPAY
                                && donHang.getTrangThaiDonHang() == TrangThaiDonHang.CHO_XU_LY
                                && donHang.getTrangThaiThanhToan() == TrangThaiThanhToan.CHO_THANH_TOAN;
        ...
        private DuLieuTaoDonHang chuanBiDuLieuTaoDonHang(
                ...
                List<ChiTietGioHang> danhSachChiTiet = chiTietGioHangRepository
                                .layDanhSachChiTietDeTaoDonHang(
                                                gioHang.getMaGioHang());

                kiemTraGioHangKhongRong(danhSachChiTiet);

                DiaChiGiaoHang diaChiGiaoHang = layDiaChiGiaoHangHopLe(
                                request.getMaDiaChi(),
                                maKhachHang);
        ...
        private void kiemTraGioHangKhongRong(
                ...
                boolean coSoLuongKhongHopLe = danhSachChiTiet.stream()
                                .anyMatch(chiTiet -> chiTiet.getSoLuong() == null
        ...
        private void kiemTraKhachHangKhongTuDatThuocKeDon(
                        List<ChiTietGioHang> danhSachChiTiet) {
                for (ChiTietGioHang chiTiet : danhSachChiTiet) {
                        if (chiTiet == null
                                        || chiTiet.getDonViSanPham() == null
                                        || chiTiet.getDonViSanPham().getSanPham() == null) {
                                throw new ResponseStatusException(
                                                HttpStatus.BAD_REQUEST,
                        ...
                        if (Boolean.TRUE.equals(
                                ...
                                throw new ResponseStatusException(
                                                HttpStatus.CONFLICT,
                                                "Khách hàng không thể tự đặt thuốc kê đơn. "
        ...
        private void kiemTraTonKhoMoiNhat(
                ...
                List<Long> danhSachMaSanPham = danhSachChiTiet.stream()
                                .map(ChiTietGioHang::getDonViSanPham)
                                .map(DonViSanPham::getSanPham)
                                .map(sanPham -> sanPham.getMaSanPham())
                                .distinct()
                                .toList();
                ...
                for (ChiTietGioHang chiTiet : danhSachChiTiet) {
                        DonViSanPham donViBan = chiTiet.getDonViSanPham();

                        Long maSanPham = donViBan.getSanPham()
                                        .getMaSanPham();
                ...
                boolean duTon = tonKhoSanPhamService.kiemTraDuTon(
                                tongCanTheoSanPham,
                                tonTheoSanPham);
        ...
        private List<DonViSanPham> layDanhSachDonViKhongTrung(
                        List<ChiTietGioHang> danhSachChiTiet) {
        ...
        private List<ChiTietDonHang> taoDanhSachChiTietDonHang(
                ...
                for (ChiTietGioHang chiTietGioHang : danhSachChiTietGioHang) {
                        ...
                        BigDecimal soLuong = BigDecimal.valueOf(
        ...
        private BigDecimal tinhTongTienHang(
                ...
                for (ChiTietDonHang chiTiet : danhSachChiTietDonHang) {
                        BigDecimal tongGiaGocDong = chiTiet.getDonGia().multiply(
                                        BigDecimal.valueOf(
                                                        chiTiet.getSoLuong()));
        ...
        private BigDecimal tinhTongGiamGiaSanPham(
                ...
                for (ChiTietDonHang chiTiet : danhSachChiTietDonHang) {
                        if (chiTiet.getGiamGia() != null) {
                                tongGiamGiaSanPham = tongGiamGiaSanPham.add(
                                                chiTiet.getGiamGia());
                        }
                }
        ...
        private DonHangResponseDto taoDonHangResponse(
                ...
                List<ChiTietDonHangResponseDto> danhSachResponse = new ArrayList<>();
        ...
        private Map<Long, List<ChiTietDonHang>> gomChiTietTheoMaDonHang(
                        List<ChiTietDonHang> danhSachChiTiet) {
                Map<Long, List<ChiTietDonHang>> ketQua = new HashMap<>();

                for (ChiTietDonHang chiTiet : danhSachChiTiet) {
                        Long maDonHang = chiTiet.getDonHang().getMaDonHang();

                        ketQua.computeIfAbsent(
                                        maDonHang,
                                        key -> new ArrayList<>()).add(chiTiet);
                }
        ...
        private DonHangDanhSachDto taoDonHangDanhSachDto(
                ...
                ChiTietDonHang chiTietDauTien = danhSachChiTiet.get(0);

                SanPhamDonHangTomTatDto sanPhamDauTien = new SanPhamDonHangTomTatDto(
                ...
                int soSanPhamKhac = danhSachChiTiet.size() - 1;

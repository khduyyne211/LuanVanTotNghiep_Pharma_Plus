...
public class PhieuNhapService {
    ...
    public PhieuNhapResponse xacNhanNhapKho(long maPhieuNhap) {
        ...
        if (phieuNhap.getTrangThaiPhieuNhap() != TrangThaiPhieuNhap.CHO_XAC_NHAN) {
            throw new IllegalArgumentException("Chỉ được xác nhận phiếu nhập đang chờ xác nhận");
        }

        List<ChiTietPhieuNhap> danhSachChiTiet = phieuNhap.getDanhSachChiTietPhieuNhap();

        if (danhSachChiTiet == null || danhSachChiTiet.isEmpty()) {
            throw new IllegalArgumentException("Phiếu nhập không có sản phẩm để xác nhận");
        }

        LocalDate ngayHienTai = LocalDate.now();

        for (ChiTietPhieuNhap chiTiet : danhSachChiTiet) {
            if (chiTiet.getHanSuDung() == null || !chiTiet.getHanSuDung().isAfter(ngayHienTai)) {
        ...
        for (ChiTietPhieuNhap chiTiet : danhSachChiTiet) {
            chiTiet.setSoLuongConLaiTheoQuyDoi(chiTiet.getSoLuongTheoQuyDoi());

            chiTiet.setTrangThaiLo(TrangThaiLo.DANG_SU_DUNG);
        }

        phieuNhap.setTrangThaiPhieuNhap(TrangThaiPhieuNhap.DA_NHAP);
    ...
    @Transactional
    public PhieuNhapResponse taoPhieuNhap(Long maNhanVienDangNhap, PhieuNhapTaoMoiRequest request) {

        NhaCungCap nhaCungCap = timNhaCungCapDangHopTac(request.getMaNhaCungCap());

        NhanVienNoiBo nhanVienLap = timNhanVienDangLamViec(maNhanVienDangNhap);
        ...
        for (ChiTietPhieuNhapTaoMoiRequest chiTietRequest : request.getDanhSachChiTiet()) {
            ...
            SanPham sanPham = timSanPhamDangHoatDong(chiTietRequest.getMaSanPham());

            DonViSanPham donViNhap = timDonViDuocPhepNhap(sanPham, chiTietRequest.getMaDonViSanPham());

            BigDecimal soLuongTheoQuyDoi = quyDoiVeDonViCoSo(sanPham, donViNhap,
                    chiTietRequest.getSoLuongNhap());

            BigDecimal thanhTien = chiTietRequest.getSoLuongNhap().multiply(chiTietRequest.getDonGiaNhap())
            ...
            chiTiet.setSoLuongNhap(chiTietRequest.getSoLuongNhap().setScale(SCALE_SO_LUONG, RoundingMode.HALF_UP));

            chiTiet.setSoLuongTheoQuyDoi(soLuongTheoQuyDoi);

            chiTiet.setSoLuongConLaiTheoQuyDoi(new BigDecimal("0.000"));

            chiTiet.setDonGiaNhap(chiTietRequest.getDonGiaNhap().setScale(2, RoundingMode.HALF_UP));
        ...
        phieuNhap.setTongTien(tongTien.setScale(2, RoundingMode.HALF_UP));
    ...
    public PhieuNhapResponse huyPhieuNhap(long maPhieuNhap) {
        ...
        PhieuNhapKho phieuNhap = phieuNhapKhoRepository.findChiTietByMaPhieuNhap(maPhieuNhap)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy phiếu nhập"));

        if (phieuNhap.getTrangThaiPhieuNhap() != TrangThaiPhieuNhap.CHO_XAC_NHAN) {
            throw new IllegalArgumentException("Chỉ được hủy phiếu nhập đang chờ xác nhận");
        }

        phieuNhap.setTrangThaiPhieuNhap(TrangThaiPhieuNhap.DA_HUY);

        return toResponse(phieuNhapKhoRepository.save(phieuNhap), true);
    }
    ...
    private NhaCungCap timNhaCungCapDangHopTac(Long maNhaCungCap) {
        ...
        if (!Boolean.TRUE.equals(nhaCungCap.getTrangThaiHopTac())) {
            throw new IllegalArgumentException("Nhà cung cấp đã ngừng hợp tác");
        }
    ...
    private NhanVienNoiBo timNhanVienDangLamViec(Long maNhanVien) {
        ...
        if (maNhanVien == null) {
            throw new IllegalArgumentException("Không xác định được nhân viên lập phiếu");
        }

        NhanVienNoiBo nhanVien = nhanVienNoiBoRepository.findById(maNhanVien)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy nhân viên lập phiếu"));

        boolean taiKhoanDangHoatDong = nhanVien.getTaiKhoan() != null
                && Boolean.TRUE.equals(nhanVien.getTaiKhoan().getTrangThaiTaiKhoan());

        if (!Boolean.TRUE.equals(nhanVien.getTrangThaiLamViec()) || !taiKhoanDangHoatDong) {

            throw new IllegalArgumentException("Nhân viên lập phiếu không còn hoạt động");
        }
    ...
    private SanPham timSanPhamDangHoatDong(Long maSanPham) {
        ...
        if (!Boolean.TRUE.equals(sanPham.getTrangThaiSanPham())) {
            ...
            throw new IllegalArgumentException("Sản phẩm đã ngừng hoạt động");
        }

        return sanPham;
    }

    private DonViSanPham timDonViDuocPhepNhap(SanPham sanPham, Long maDonViSanPham) {

        DonViSanPham donViSanPham = donViSanPhamRepository.findById(maDonViSanPham)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy đơn vị nhập của sản phẩm"));

        Long maSanPhamCuaDonVi = donViSanPham.getSanPham() != null ? donViSanPham.getSanPham().getMaSanPham()
                : null;

        if (!Objects.equals(maSanPhamCuaDonVi, sanPham.getMaSanPham())) {

            throw new IllegalArgumentException("Đơn vị nhập không thuộc sản phẩm đã chọn");
        }

        if (!Boolean.TRUE.equals(donViSanPham.getTrangThai())) {

            throw new IllegalArgumentException("Đơn vị nhập đã ngừng hoạt động");
        }

        if (!Boolean.TRUE.equals(donViSanPham.getChoPhepNhap())) {

            throw new IllegalArgumentException("Đơn vị này không được phép nhập kho");
        }

        return donViSanPham;
    }

    private BigDecimal quyDoiVeDonViCoSo(SanPham sanPham, DonViSanPham donViNhap, BigDecimal soLuongNhap) {

        List<DonViSanPham> danhSachDonVi = donViSanPhamRepository.findBySanPham_MaSanPham(sanPham.getMaSanPham());

        List<DonViSanPham> danhSachDonViCoSo = danhSachDonVi.stream()
        ...
        if (danhSachDonViCoSo.size() != 1) {
            throw new IllegalArgumentException("Sản phẩm phải có đúng một đơn vị cơ sở đang hoạt động");
        }

        Long maDonViNhap = donViNhap.getMaDonViSanPham();

        Long maDonViCoSo = danhSachDonViCoSo.get(0).getMaDonViSanPham();

        if (Objects.equals(maDonViNhap, maDonViCoSo)) {

            return soLuongNhap.setScale(SCALE_SO_LUONG, RoundingMode.HALF_UP);
        }
        ...
        hangDoi.add(new NutQuyDoi(maDonViNhap, BigDecimal.ONE));
        ...
        while (!hangDoi.isEmpty()) {
            ...
            for (CanhQuyDoi canh : doThiQuyDoi.getOrDefault(nutHienTai.maDonVi(), List.of())) {

                if (!donViDaDuyet.add(canh.maDonViTiepTheo())) {
                    continue;
                }

                BigDecimal heSoMoi = nutHienTai.heSoTichLuy().multiply(canh.heSo());

                if (Objects.equals(canh.maDonViTiepTheo(), maDonViCoSo)) {

                    return soLuongNhap.multiply(heSoMoi).setScale(SCALE_SO_LUONG, RoundingMode.HALF_UP);
                }

                hangDoi.add(new NutQuyDoi(canh.maDonViTiepTheo(), heSoMoi));
            }
        }

        throw new IllegalArgumentException("Không tìm thấy quy đổi từ đơn vị nhập về đơn vị cơ sở");
    }

    private Map<Long, List<CanhQuyDoi>> taoDoThiQuyDoi(Long maSanPham) {
        ...
        for (QuyDoiDonVi quyDoi : danhSachQuyDoi) {
            ...
            if (donViNguon == null || donViDich == null || !Boolean.TRUE.equals(donViNguon.getTrangThai())
            ...
            themCanh(doThi, donViDich.getMaDonViSanPham(), donViNguon.getMaDonViSanPham(), heSoNghich);
        }

        return doThi;
    }

    private void themCanh(Map<Long, List<CanhQuyDoi>> doThi, Long maDonViNguon, Long maDonViDich, BigDecimal heSo) {

        doThi.computeIfAbsent(maDonViNguon, khoa -> new ArrayList<>()).add(new CanhQuyDoi(maDonViDich, heSo));
    }

    private String taoKhoaChiTiet(ChiTietPhieuNhapTaoMoiRequest request) {

        return request.getMaSanPham() + "|" + request.getMaDonViSanPham() + "|" + request.getHanSuDung();
    }

    private String chuanHoaGhiChu(String ghiChu) {

        return ghiChu == null || ghiChu.isBlank() ? null : ghiChu.trim();
    }

    private PhieuNhapResponse toResponse(PhieuNhapKho phieuNhap, boolean baoGomChiTiet) {
        ...
        List<ChiTietPhieuNhapResponse> danhSachChiTiet = baoGomChiTiet
                ? phieuNhap.getDanhSachChiTietPhieuNhap().stream().map(this::toChiTietResponse).toList()
                : List.of();

        return PhieuNhapResponse.builder().maPhieuNhap(phieuNhap.getMaPhieuNhap())
                .maNhaCungCap(nhaCungCap != null ? nhaCungCap.getMaNhaCungCap() : null)
                .tenNhaCungCap(nhaCungCap != null ? nhaCungCap.getTenNhaCungCap() : null)
                .maNhanVienLap(nhanVienLap != null ? nhanVienLap.getMaNhanVien() : null)
                .tenNhanVienLap(nhanVienLap != null ? nhanVienLap.getHoTen() : null)
                .ngayNhap(phieuNhap.getNgayNhap()).tongTien(phieuNhap.getTongTien())
                .trangThaiPhieuNhap(phieuNhap.getTrangThaiPhieuNhap()).ghiChu(phieuNhap.getGhiChu())
                .danhSachChiTiet(danhSachChiTiet).build();
    }

    private ChiTietPhieuNhapResponse toChiTietResponse(ChiTietPhieuNhap chiTiet) {

        SanPham sanPham = chiTiet.getSanPham();

        DonViSanPham donViSanPham = chiTiet.getDonViSanPham();

        DonViTinh donViTinh = donViSanPham != null ? donViSanPham.getDonViTinh() : null;

        return ChiTietPhieuNhapResponse.builder().maChiTietPhieuNhap(chiTiet.getMaChiTietPhieuNhap())
                .maSanPham(sanPham != null ? sanPham.getMaSanPham() : null)
                .tenSanPham(sanPham != null ? sanPham.getTenSanPham() : null)
                .maDonViSanPham(donViSanPham != null ? donViSanPham.getMaDonViSanPham() : null)
                ...
                .soLuongNhap(chiTiet.getSoLuongNhap()).soLuongTheoQuyDoi(chiTiet.getSoLuongTheoQuyDoi())
                .soLuongConLaiTheoQuyDoi(chiTiet.getSoLuongConLaiTheoQuyDoi()).donGiaNhap(chiTiet.getDonGiaNhap())
                .thanhTien(chiTiet.getThanhTien()).hanSuDung(chiTiet.getHanSuDung()).trangThaiLo(chiTiet.getTrangThaiLo())
                .build();
    }

    private record CanhQuyDoi(Long maDonViTiepTheo, BigDecimal heSo) {
    }

    private record NutQuyDoi(Long maDonVi, BigDecimal heSoTichLuy) {
    }
}